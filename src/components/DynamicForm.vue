<template>
    <el-form ref="ElForm" :model="formModel" :label-width="labelWidth+'px'" :label-position="labelAlign" :inline="true">
        <el-form-item v-for="(item,index) in labelData"
                      :label="item.label.label_name_chs"
                      :style="{width:getFormItemWidth()+'px'}"
                      :prop="item.label_id+''"
                      :rules="rules[index]"
                      v-if="item.label.label_type === 0">
...
<script>
    export default {
        ...
        methods: {
                        ...
                        if (_label.label_config.control_config.control_type === 'select' || _label.label_config.control_config.control_type === 'radio' || _label.label_config.control_config.control_type === 'checkbox') {
                            if (_label.label_config.data_source.data_source_type === 0) {
                                this.getDicSource(_label.label_config.data_source.dic_type_id, (res) => {
                                    let _data = []
                                    let _ids = _label.label_config.data_source.dic_par_ids.split(',')
                                    _ids.forEach(item => {
                                ...
                                let _data = []
                                let dataSource = _label.label_config.data_source.value.split(_label.label_config.data_source.separtor)
                                ...
                                let param = []
                                for (let pro in _label.label_config.data_source.request_parameter_map) {
                                    param.push({
                                        parameter_name: pro,
            ...
            //获取Api数据
            getDataSource(api_id, _param, callBack) {
                let param = {
                    apiId: api_id,
                    request: _param
                }
                this.$post('/api/Data/ListData', param).then(
                    res => {
                        callBack(res)
                    }
                )
            ...
            getLabelControlOptions(index,key) {
                let value = null
                let options = this.labelData[index].label.label_config.control_config.control_options
            ...
            autocompleteSearch(index) {
                return (queryString, callback) => {
                    let _label = this.labelData[index].label
                    let param = []
                    for (let pro in _label.label_config.data_source.request_parameter_map) {
                        param.push({
                            parameter_name: pro,
            ...
            submitForm(fn) {
                this.$refs.ElForm.validate(valid => {
                    if (valid) {
                        ...
                        this.$post('/api/Form/Submit', param).then(
                            res => {
